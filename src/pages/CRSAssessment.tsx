...
const CRSAssessment = () => {
  ...
  const handleComplete = (formData: any) => {
    // Use the calculated scores from the questionnaire
      ...
      breakdown: {
        coreHumanCapital: {
          ...
          details: {
            ...
            language: 0, // Would come from language questions
          }
        },
        spouseFactors: {
  ...
  // Determine best matching category based on score and profile
  const bestCategory = useMemo(() => {
    ...
    // Find category where score is closest to or above cutoff
    ...
    const sorted = eligibleCategories.sort((a, b) => {
      ...
      // Prefer categories where user is above cutoff
      if (aMargin >= 0 && bMargin < 0) return -1;
      if (bMargin >= 0 && aMargin < 0) return 1;
      // Otherwise prefer closest to cutoff
  ...
  // Calculate days until expected next draw
